...
if ! command -v dpkg >/dev/null 2>&1; then
    echo "This script only supports Debian/Ubuntu (dpkg not found)."
    exit 1
fi
...
if ! dpkg --get-selections | awk '{print $1}' > installed_apps.txt; then
...
# safer cleanup (don’t blindly nuke everything)
